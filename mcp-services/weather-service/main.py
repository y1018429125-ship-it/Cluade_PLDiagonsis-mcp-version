"""FastAPI service entry for the weather fault diagnosis tool.

This service exposes the same HTTP interface as the other PLDiagnosis
MCP services. It consumes the getWeather API (temperature and humidity
only) and outputs a pure language description — no confidence or weight
is produced for the PLD weighted diagnosis.
"""
...
async def diagnose(req: DiagnoseRequest) -> DiagnoseResponse:
    """Fetch fault-time temperature/humidity and describe the humidity level."""
    client = APIClient()
    try:
        ...
        structured_data: dict[str, Any] = {
            ...
            "humidity_description": report["humidity_description"],
            "details": {
        ...
        logger.error(f"气象查询失败: {exc.message}")
        raise HTTPException(status_code=500, detail=exc.message)
    except Exception as exc:
        logger.error(f"未知错误: {exc}")
        raise HTTPException(status_code=500, detail=f"气象查询过程中发生未知错误: {exc}")
    finally:
        await client.close()
